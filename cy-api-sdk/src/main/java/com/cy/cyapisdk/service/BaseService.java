...
import com.cy.cyapisdk.exception.ErrorCode;
import com.cy.cyapisdk.exception.ErrorResponse;
import com.cy.cyapisdk.model.request.BaseRequest;
import com.cy.cyapisdk.model.response.ResultResponse;
import com.google.gson.Gson;
import com.google.gson.JsonSyntaxException;
import com.google.gson.reflect.TypeToken;
import com.cy.cyapisdk.client.CyApiClient;
import com.cy.cyapisdk.utils.SignUtils;
import lombok.Data;
...
public abstract class BaseService implements ApiService {
    private CyApiClient cyApiClient;
    /**
     * 网关HOST
     */
     ...
     */
    public void checkConfig(CyApiClient cyApiClient) throws ApiException {
        if (cyApiClient == null && this.getCyApiClient() == null) {
            throw new ApiException(ErrorCode.NO_AUTH_ERROR, "请先配置密钥AccessKey/SecretKey");
        }
        if (cyApiClient != null && !StringUtils.isAnyBlank(cyApiClient.getAccessKey(), cyApiClient.getSecretKey())) {
            this.setCyApiClient(cyApiClient);
        }
    }
    ...
    private <O, T extends ResultResponse> HttpRequest getHttpRequestByRequestMethod(BaseRequest<O, T> request) throws ApiException {
        ...
        return httpRequest.addHeaders(getHeaders(JSONUtil.toJsonStr(request), cyApiClient)).body(JSONUtil.toJsonStr(request.getRequestParams()));
    }
     ...
     */
    private Map<String, String> getHeaders(String body, CyApiClient cyApiClient) {
        Map<String, String> hashMap = new HashMap<>(4);
        hashMap.put("accessKey", cyApiClient.getAccessKey());
        String encodedBody = SecureUtil.md5(body);
        hashMap.put("body", encodedBody);
        hashMap.put("timestamp", String.valueOf(System.currentTimeMillis() / 1000));
        hashMap.put("sign", SignUtils.getSign(encodedBody, cyApiClient.getSecretKey()));
        return hashMap;
    }
    ...
    @Override
    public <O, T extends ResultResponse> T request(CyApiClient cyApiClient, BaseRequest<O, T> request) throws ApiException {
        checkConfig(cyApiClient);
